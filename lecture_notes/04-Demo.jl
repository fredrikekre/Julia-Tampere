...
# ╔═╡ 69c1f834-2d21-11ee-3171-7f7d29c74b9b
begin
	using PlutoUI, Plots, DifferentialEquations, Ferrite
	
	nothing
end

# ╔═╡ 8bfa7e26-782a-42f1-9c71-3b167f573ac5
begin
	md"""
	**Let's solve an initial value problem**

	``\dot{u} = f(u,t) =`` $( @bind fstring TextField(;default="u") )

	``t \in`` $( @bind trange RangeSlider(0:0.1:10; default=0:1))

	``u_0 =`` $( @bind u₀ NumberField( 0:10; default=1) )
	"""
end

# ╔═╡ 9a3e947c-5b47-48ab-9dea-e0a64a6be571
begin
	#solvers = [Tsit5(), AutoTsit5(Rosenbrock23()), BS3(), Rodas4()]
	solvers = [Tsit5()=>"Tsit5", AutoTsit5(Rosenbrock23())=>"AutoTsit5", BS3()=>"BS3", Rodas4()=>"Rodas4"]
	defaultsolver = Tsit5()
	md"""
	Utilized solver: $( @bind solver Select(solvers) )
	"""
end

# ╔═╡ d90abfe6-bb47-4b2d-9783-0aa0da7a98b9
begin
	f = eval(Meta.parse("(u, p, t) -> " * fstring))
	
	prob = ODEProblem(f, u₀, (trange[1], trange[end]))
	
	tsolve = @elapsed solution = solve(prob, solver)

	tplot = @elapsed p = plot(solution; linewidth=3, title = "Solution", xaxis = "t", yaxis = "u", label=false)

	md"""
	$(p)
	
	Time for computing solution: $(tsolve) s

	Time for creating plot: $(tplot) s
	"""
end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "9d4beec941d9958fddb700cc5d6a625aa6a094b2"
...
git-tree-sha1 = "90bf2a84269946bd82651291fe6ff1f1967a2615"
