...
# ╔═╡ 3259fce0-35f8-11ee-266e-919ea7e75230
begin
...
# ╔═╡ e088a99c-86b7-4a99-92a5-eacd4fc4c0b2
begin
	md"""
	## Task 1: Line-plots of functions

	``f_1(x) =`` $( @bind f₁string TextField(;default="x") )

	``f_2(x) =`` $( @bind f₂string TextField(;default="x^2") )

	``x \in`` $(@bind xrange RangeSlider(-10:10; default=-5:5))
	"""
end

# ╔═╡ c063430b-3d26-4a59-9a00-e56540780ffe
begin
	f₁ = eval(Meta.parse("x -> " * f₁string))
	f₂ = eval(Meta.parse("x -> " * f₂string))
	plotrange = LinRange(minimum(xrange), maximum(xrange), 100)

	fig = Figure()
	ax = Axis(fig[1,1], ylabel = "f", xlabel = "x")
	
	lines!(ax, plotrange, f₁; label="f₁", color=:blue)
	lines!(ax, plotrange, f₂; label="f₂", color=:orange)

	Legend(fig[1,2], ax)
	fig
end

# ╔═╡ d8d19270-c375-4c49-8254-c649013ab3b9
begin
	fig₂ = Figure()
	ax₂ = (Axis(fig₂[1,1], ylabel = "f", xlabel = "x", title="f₁"),
	       Axis(fig₂[2,1], ylabel = "f", xlabel = "x", title="f₂"))
	
	lines!(ax₂[1], plotrange, f₁; color=:blue)
	lines!(ax₂[2], plotrange, f₂; color=:blue)

	fig₂
end

# ╔═╡ 0dcd0c03-bccd-4d65-ba8a-0f0c1cb5efe9
begin
	md"""
	## Task 2: Scatter-plot of random values
	
	``n =`` $(@bind n PlutoUI.Slider(10:100; default=20, show_value=true))
	"""
end

# ╔═╡ d2afc232-c80d-4169-b0ae-bf80bf4229d4
begin
	x, y = rand(n), rand(n)

	fig2 = Figure()
	ax2 = Axis(fig2[1,1], ylabel = "y", xlabel = "x", title="Random values")
	
	scatter!(ax2, x,y; label=false)

	fig2
end

# ╔═╡ e0d938c3-4b0a-40d7-829d-ccb0b28b602a
begin
	# See makie docs for one example
	options = [sin, cos]
	
	md"""
	## Task 3

	Selected function: $( @bind u Select(options; default=options[1]) )
	"""
end

# ╔═╡ 48d073a7-2ecb-47ee-a25e-599ebc12c18c
begin
	t = Observable([0.0])
	z = @lift sin.( $t )

	fig3 = Figure()
	ax3 = Axis(fig3[1,1]; limits=(0,10,-1,1))
	p = lines!(ax3, t, z)
	
	anim = record(fig3, "animation.gif", 0:0.1:10) do newt
    	t[] = collect(0:0.1:newt)
	end

	LocalResource("animation.gif")
end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "52e99e4e8f746a26964a1549ea93b7fb63dc98cc"
